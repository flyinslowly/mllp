...
class BO(object):
    ...
    def __init__(self, model, space, objective, acquisition, evaluator, X_init, Y_init=None, cost = None, normalize_Y = True, model_update_interval = 1, de_duplication = False):
        ...
        self.sigma_arr = []
        self.X_val, self.Y_val = np.meshgrid(np.arange(-1, 1, 0.01), np.arange(-1, 1, 0.01))
    ...
    def run_optimization(self, max_iter = 0, max_time = np.inf,  eps = 1e-8, context = None, verbosity=False, save_models_parameters= True, report_file = None, evaluations_file = None, models_file=None, test_X = None):
        """
        ...
        while (self.max_time > self.cum_time):
            ...
            #print('sugg_sample: ', self.suggested_sample)
            ...
            # 2D_case
            #self.sigma_arr.append(np.dstack((self.model.predict(np.dstack((self.X_val, self.Y_val))[k])[1] for k in range(self.X_val.shape[0]))).reshape(200, 200))

            prediction = self.model.predict(self.X)
            #print('mean: ', self.model.predict(self.X)[0])
            self.predictions_list.append(self.model.predict(self.suggested_sample)[0])
            self.error_list.append(np.mean(prediction[1] / prediction[0]))

            if test_X != None:
                predict_test = self.model.predict(test_X)
                self.test_error_list.append(np.mean(predict_test[1]/predict_test[0]))
